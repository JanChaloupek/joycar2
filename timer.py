...
class Timer:
    ...
    def getTimeDiff(self, test_time_ms:int) -> int:
        # vrati rozdil casu od startu casovace
        return ticks_diff(self.__getTime(test_time_ms), self.__startTime)

    def isTimeout(self, test_time_ms:int=None, timeout_ms:int=None) -> bool:
        # vyprsel timeout casovace v case 'time_ms'?
        if not self.isStarted():
            return False
        diff_ms = self.getTimeDiff(test_time_ms)
